      ...
      <Table>
        ...
        <TableBody>
          {products?.map((product) => {
            if(product)
            {
              const { _id, id, name, size, price, color, cover, quantity, available, image, status } = product;
              return (
                <TableRow key={id}>
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Image alt="product image" src={`${baseUrl}/${image}`} sx={{ width: 64, height: 64, borderRadius: 1.5, mr: 2 }} />
                      <Box>
